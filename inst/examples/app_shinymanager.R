...
#> STEP: Build the UI behind a login
#> NOTE: The app's own page, tabbed by table, then wrapped by
#> NOTE: shinymanager::secure_app(): nobody reaches a form_ui() before logging
#> NOTE: in. enable_admin = TRUE adds shinymanager's own user administration.
#> NOTE: Note what is NOT here: no permission logic. The UI is the same for
#> NOTE: everyone; what each user may do is decided server-side from the rights
#> NOTE: table, and hide_forbidden then removes the controls they lack.
app_page <- fluidPage(
  titlePanel("shinyformtools + shinymanager: a rights table"),
  p(
  ...
  tabsetPanel(
    tabPanel("Tickets", form_ui("tickets", title = "Tickets")),
    tabPanel("Agents", form_ui("agents", title = "Agents")),
    tabPanel("Permissions", uiOutput("permissions_panel"))
  )
)
  ...
  output$permissions_panel <- shiny::renderUI({
    if (is_admin()) {
      shiny::tagList(
        ...
        form_ui("permissions", title = "Rights table")
      )
    } else {
      shiny::p(
...
#> DEMO
# The walkthrough beside the app; not part of the application. secure_app()
# owns a full-screen login overlay that cannot be split across columns, so the
# walkthrough is spliced in one level down, INSIDE the page secure_app wraps:
# once logged in, the step cards show on the left and the running app on the
# right. Hence the re-wrap rather than demo_page(ui, ...).
source(example_path("_demo_scaffold"), local = TRUE)
ui <- shinymanager::secure_app(
  demo_page(app_page, "app_shinymanager"),
  tags_top = login_banner,
  enable_admin = TRUE
)
#> DEMO END
