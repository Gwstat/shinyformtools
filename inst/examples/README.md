...
- **app_custom_input** — extending the input set with `register_input()`. Two
  `shinyWidgets` inputs are registered and then used as ordinary `input_type`s in
  `form_field()`: a single-valued `knobInput` dial (stored in a `REAL` column,
  with `updateKnobInput` for dynamic values) and a multi-valued `pickerInput`
  (`multiple = TRUE`, so the selection is stored as a JSON array and shown joined
  in the table). Once registered, the widgets render, validate, store and restore
  exactly like a built-in. Requires the `shinyWidgets` package.
