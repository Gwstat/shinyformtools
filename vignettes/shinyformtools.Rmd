...
The 18 runnable examples shipped with the package cover the individual features;
...
Only the CRUD core is displayed by default; extras are opt-in on `form_ui()`,
...
or to `form_server(user = ...)` -- the package does not invent an identity. If
you pass no user, the three `_by` columns stay `NULL` and cost nothing.

The record's identity is `sft_id`: a primary key the *database* assigns, so it is
unique under concurrent writes on every backend. Two further identifiers are
available but off by default, because neither adds anything `sft_id` does not
already guarantee:

| Opt-in | What it buys |
|---|---|
| `form(uuid = TRUE)` | `sft_uuid`, an identifier that stays stable when records move between databases, and the `record_uuid` argument of the CRUD and audit functions. Costs 36 bytes per record -- often more than the record's own data. |
| `form(easy_id = TRUE)` | `sft_easy_id`, a short quotable id (`sft_id` plus two random letters, e.g. `1-QZ`). Nicer to read out loud; not a stronger identifier. Shown instead of `sft_id` in the records table. |
...
- **`sft_schema_migrations`** -- one row per schema action, read with
  `fetch_schema_migrations()`. See below.
- **`sft_user_preferences`** -- per-user UI state, such as saved column views.
  Nothing about your data.

### The schema has a history too

`sft_audit_log` records changes to your data; `sft_schema_migrations` records
changes to its shape. `fetch_schema_migrations()` reads it:

```{r schema-history}
fetch_schema_migrations(contacts, conn = conn)[
  , c("from_version", "to_version", "action", "db_column", "applied_by")
]
```

One row so far -- the table being created. It earns its keep later. Suppose you
drop a field from the form: the package will *not* drop its column, because that
would throw the data away. It marks the column retired and leaves it in place. A
year on, you find a column no form declares, holding data nobody asked for, and
the database cannot tell you where it came from. This log can: which version
added it, which retired it, and who ran each.

`from_version` is `NA` for the first action, since there is no version to come
from.
...
live[, c("sft_id", "name", "email", "role", "sft_updated_by")]
...
DBI::dbGetQuery(
  conn,
  "SELECT sft_id, name, role, joined
   FROM contacts
